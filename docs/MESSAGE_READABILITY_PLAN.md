...
> ## 🔵 아직 유효하다 (2026-08-26 확인)
>
> 미구현이고, **여기 적힌 진단이 실제 결함으로 드러났다.** §2.1의 「체크리스트 항목 텍스트는
> 생성 시점에 고정된 문자열이다」가 바로 도착 항목이 낡는 원인이다 — 착대 번호가 압축으로
> 밀리면 `3착대 도착` 이 다른 편성을 가리키거나 갈 곳을 잃는다.
>
> 2026-08-26 에 **증상만** 가렸다(편성이 없으면 `(편성없음)` 표시). 텍스트를 파생값으로
> 바꾸는 이 문서의 방안은 그대로 남아 있다.
