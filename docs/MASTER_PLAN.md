...
| # | 결정할 것 | 선택지 | 근거 |
|---|---|---|---|
| ~~**A-1**~~ | 로그에 **「누가 결정했는가」(actor)** 를 남길 것인가 | ✅ **A 안 남긴다 — 확정(2026-09-15 사용자 결정).** 결정은 모두 지휘관이 한 것이라 따로 기록할 필요가 없다. 스키마의 `actor` 자리는 채우지 않는다 | [EVENT_LOG_PLAN.md](EVENT_LOG_PLAN.md) §8 미결 2 |
...
~~A-1 이 E-3 이후 로그 작업 전체를 막고 있다.~~ **A-1 은 「남기지 않는다」로 풀렸다(2026-09-15)** — 로그 작업을 막던 것이 없다.
스키마에 열어 둔 `actor` 자리(E-3-8)는 쓰지 않는다.

**설정(시나리오)을 PC 파일에 저장한다 — 범위 확장(2026-09-16 사용자 결정).** 설정모드를 고치는 일이라 D-4 의
현재 범위(훈련모드 무플) 밖이지만 사용자가 직접 정했다. localStorage 는 주소마다 따로라 태블릿 · 다른 포트에서
목록이 비고, 사이트 데이터를 지우면 사라졌다. 서버(`serve-dist.mjs` · 개발 서버)가 `data/settings.json` 에 두고
앱은 열 때 받아 사본에 깐다 — [DATA_FLOW.md](DATA_FLOW.md) §2.

**대기1단계 미운영 = A면 도착은 의도한 동작이다(2026-09-15 사용자 확인).** 대기1단계의 기본은 미운영이라, 운영을
누르기 전에는 시간 도착 · 더블클릭 모두 A면으로 들어온다(`utils/dispatchTarget.ts`, `tests/dispatchTarget.test.mjs`).
...
| **B-11** | **송수라인 임무 자동지정 + 순환보수 칸** | 중간 | ✅ **구현·브라우저 검증 완료 (09-09).** 「중요」·「1선」을 연결에서 파생시키고(수동 지정은 없앤다), 거리 150m 이상 소화전에 5대까지 쌓는 「순환」 칸을 만든다. **순환 차량은 개별 송수라인을 갖지 않고 칸에서 나가는 선 하나가 무리 전체의 송수다** — [WATER_SUPPLY_MISSION_PLAN.md](WATER_SUPPLY_MISSION_PLAN.md) §4.1. 곁다리로 **고장난 소화전이 계속 물을 주는 결함**(같은 문서 §2.1)을 함께 고친다 |
