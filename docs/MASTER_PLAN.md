...
| **B-7** | `--ui-scale` 잔재 정리 | 작음 | 훈련모드 CSS 에 참조 94건. **정의가 없어 전부 폴백 1 로 죽어 있다** |
| **B-8** | `ZoneCell` 무효 CSS | **2줄** | `outline: … inset` 은 선언 전체가 무시된다 — 우측·중앙 구역 hover 외곽선이 **지금 그려지지 않는다.** 고칠 방법이 둘이라(`inset` 제거 / `box-shadow`) 원래 의도 확인이 필요하다 — **P-9** |
