...
### 2.3 품질 기준선 (2026-08-18 실측)

| 항목 | 값 | 판정 |
|---|---|---|
| `npx tsc -b --force` | 통과 (exit 0) | ✅ |
| `npx eslint .` | 56 errors / 12 warnings | ✅ 기준선 유지 (`CLAUDE.md` "약 56건") |
| 코드 규모 | 117 파일 / 22,362행 | |
...
| # | 작업 |
|---|---|
| 5-1 | `DATA_FLOW.md` 갱신 — Provider 순서(§12), `runtime.*` 키 11종, 좌표 정규화 반영 |
| 5-2 | `FEATURE_STATUS.md` 갱신 — 현재 구현 상태를 **네 모드 구분**(D-4)에 맞춰 재작성 |
| 5-3 | `PROJECT_OVERVIEW.md` 갱신 — 폴더 구조·화면 구성. "설정창/훈련창/분석창" 3분할 서술을 네 모드로 교체 |
| 5-4 | `TODO_ROADMAP.md`에 낙후 경고 배너 + 무효 항목 표시(에러 바운더리 등) |
| 5-5 | `PROJECT_PLAN.md` §13을 "이 문서 §4로 대체됨"으로 표시 |
| 5-6 | `README.md` — 현재 Vite 기본 안내문 상태. 제품 설명으로 교체 |
